...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    year_dir = AOC_DIR / "src"

    for day, solution in enumerate(get_paths_matching_regex(year_dir, DAY_PATTERN)):
        solution_paths_dict[year][day + 1] = [solution.relative_to(AOC_DIR).as_posix()]
...
def generate_day_tile_image(day: str, year: str, languages: list[str], day_scores: Union[DayScores, None], path: Path):
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: Union[DayScores, None], needs_update: bool):
    languages = []
